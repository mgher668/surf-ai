...
- 当前决策（2026-04-06）：先不实现自动 `IDLE` 流转，后续按部署规模再补。
...
- 2026-04-06 优先级更新：先做“检索增强（语义召回 + BM25 混合排序）”。
- 2026-04-06 已完成：会话内混合检索排序（BM25 + 轻量语义相似度）并保留证据序号绑定。
...
- 2026-04-06 下一步：补“轻量安全告警 UX”（sidepanel 状态提示 + badge + 最近审计事件快览）。
- 2026-04-06 已完成：轻量安全告警 UX（sidepanel 状态提示 + 扩展 badge + 最近审计事件快览）。
